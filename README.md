...
- `react-native-track-player` wrapper configured for background, lock-screen, and Bluetooth controls.
...
`react-native-track-player` and `react-native-pdf` require native modules, so use an Expo development build (`expo run:ios` / `expo run:android`) rather than Expo Go for full playback and PDF behavior.
